  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: seasons.map((s) {
        ...
        final name =
            s['name'] as String? ??
            (tr?.get('detail_season_label').replaceAll('{}', num.toString()) ??
                'Season $num');
        ...
        final watchedLabel = watched
            ? (tr?.get('semantics_season_watched') ?? 'Watched')
            : (tr?.get('semantics_season_unwatched') ?? 'Not watched');
        final seasonLabel =
            tr
                ?.get('semantics_toggle_season')
                .replaceFirst('{}', name)
                .replaceFirst('{}', '$eps')
                .replaceFirst('{}', watchedLabel) ??
            '$name season, $eps episodes. $watchedLabel';
        return SpringButton(
          onTap: () async {
